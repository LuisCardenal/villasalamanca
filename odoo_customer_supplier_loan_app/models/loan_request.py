...
class LoanRequest(models.Model):
    ...
    is_interest_payable = fields.Boolean(string="Is Interest Payable", default=True,
                                         related="loan_type_id.is_interest_payable")
    ...
    received_from_partner = fields.Float(string="Received From Partner", compute="_compute_balance_on_loan",
                                         store=True)  #
    balance_on_loan = fields.Float(string="Balance On Loan", compute="_compute_balance_on_loan", store=True)  #

    loan_proof_ids = fields.Many2many('loan.proof', 'rel_loan_proof_type_request', string="Loan Proofs",
                                      related="loan_type_id.loan_proof_ids")
    ...
    disburse_journal_entry_id = fields.Many2one('account.move', string="Disbure Journal Entry", readonly=True,
                                                copy=False)
    ...
    account_entery_id = fields.Many2one('account.move', string="Interest On Loan Journal Entry", readonly=True,
                                        copy=False)
    ...
    state = fields.Selection([('draft', 'Draft'), ('applied', 'Applied'), ('approve', 'Approved'), ('cancel', 'Cancel'),
                              ('disbursed', 'Disbursed')], default='draft')
    ...
    def action_confirm(self):
        ...
        if self.policy_ids:
            ...
            if value_gap > 0:
                loan_res = self.env['loan.request'].search(
                    [('partner_id', '=', self.partner_id.id), ('state', '=', 'disbursed')])
                loan_dates = []
                for emp in loan_res:
                    if emp.id == self.id:
                        continue
    ...
    def disburse_loan(self):
        ...
        credit_line = [0, 0, {'account_id': self.disburse_journal_id.id,
                              'partner_id': self.user_id.partner_id.id,
                              'name': 'Loan Of ' + name_of,
                              'debit': 0.0,
        ...
        journal.action_post()
        self.write({'state': 'disbursed', 'disburse_journal_entry_id': journal.id,
        ...
        self.write(
            {'state': 'disbursed', 'account_entery_id': int_jounral.id, 'disbursement_date': fields.datetime.now()})  #
